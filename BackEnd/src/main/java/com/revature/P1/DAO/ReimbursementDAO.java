...
import com.revature.P1.Util.Exceptions.InvalidUserInputException; // replaced for RuntimeError - MW
...
//
    ...
    public Reimbursement create(Reimbursement newReimbursement) {
        // Updated to send null to ticket_id and status, to use their default values in DB
        // Imported Exception for InvalidUserInput - MW
        try(Connection connection = ConnectionFactory.getConnectionFactory().getConnection()){

            String sql = "insert into reimbursement_ticket(user_name, description, amount) values (?, ?, ?)";
            ...
            preparedStatement.setDouble(3, newReimbursement.getAmount());

            int verifyInsert = preparedStatement.executeUpdate();
            if (verifyInsert == 0){
                throw new InvalidUserInputException("Reimbursement was not added");
            }
